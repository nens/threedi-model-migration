...
def test_load_modeldatabank(metadata_json_path):
    ...
    assert bb.meta == {"description": "foo"}

    d = metadata["dijkpolder"]
    assert d.meta == {}


def test_load_modeldatabank_with_blacklist(metadata_json_path, owner_blacklist_path):
    metadata = load_modeldatabank(metadata_json_path, owner_blacklist_path)
    assert len(metadata) == 1

    assert "dijkpolder" in metadata
